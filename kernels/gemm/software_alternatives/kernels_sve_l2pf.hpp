...
/*
 * kernels_sve_l2pf.hpp — L2 software-prefetch variant of the mock microkernel.
 *
 * Clone of sve_interleaved_fp16_mla_8x3VL, but issues a scalar A64
 *   prfm  pldl2keep, [x22, #PF_OFF_BYTES]
 * software prefetch of the B panel ahead of use. B loads stay NORMAL ld1h.
 *
 * WHY scalar prfm (not svprfh): this gem5 fork decodes scalar A64 PRFM as a
 * real timed Request::PREFETCH (ldr64.isa, flavor="mprefetch"), but decodes
 * SVE prfh as WarnUnimplemented (NOP, no memory effect). So scalar prfm is the
 * only software prefetch that is actually MODELED here.
 *
 * NOTE: the "non-temporal load" (L1-bypass) half of the substitute is NOT
 * present here, because this gem5 fork does not decode SVE LDNT1H (panics) and
 * does not model scalar LDNP as L1-bypassing (no NO_ALLOCATE request flag).
 * This variant therefore isolates ONLY the L2-software-prefetch dimension.
 *
 * PF_OFF_BYTES is a compile-time byte offset for the prefetch (sweep it).
 * Default 512 B = 8 cache lines ahead. 0 disables the prefetch.
 *
...
#define L2PF_STR2(x) #x
#define L2PF_STR(x)  L2PF_STR2(x)

#if PF_OFF_BYTES > 0
#  define L2PF_PRFM_B "prfm pldl2keep, [x22, #" L2PF_STR(PF_OFF_BYTES) "]\n"
#else
#  define L2PF_PRFM_B ""
#endif
...
void sve_interleaved_fp16_mla_8x3VL_l2pf(
