...
/*
 * kernels_sve_l2pin_nt.hpp — STOCK-ARM SPM substitute microkernel
 *   (L2-targeted prefetch + non-temporal load), the RUNNABLE-under-gem5 form.
 *
 * Clone of sve_interleaved_fp16_mla_8x3VL, identical FMA order, but B's reads
 * use the STANDARD ARMv8 instructions that approximate CacheFlex's SPM ops:
 *   - spm.ld1qd (read B from SPM)  ->  ldnt1h  (SVE non-temporal load).
 *       Architectural non-temporal hint => B does not allocate/retain in L1.
 *   - spm_cp    (install B to SPM) ->  scalar prfm pldl2keep  (prefetch to L2).
 *       Issued PF_OFF_BYTES ahead of the current B pointer x22.
 *
 * vs kernels_sve_ntpf.hpp: that header used SVE `prfh PLDL2KEEP` (svprfh), which
 * THIS gem5 fork decodes as a NOP. Here we use the SCALAR A64 `prfm pldl2keep`,
 * which the fork decodes as a real timed Request::PREFETCH (and, with the
 * fidelity fix, marks NO_L1_ALLOC so it targets L2 not L1). So this header is
 * the one that actually exercises both halves of the stock-ARM substitute.
 *
 * A-panel loads (ld1rqh) and C-stores stay NORMAL/coherent through L1.
 * Output is bit-identical to sve_interleaved_fp16_mla_8x3VL.
 *
 * PF_OFF_BYTES: byte distance of the L2 prefetch ahead of x22 (sweepable).
 *   default 512 (= 8 cache lines). 0 disables the prefetch.
 */
#pragma once
#include "kernels_sve.hpp"   // sve_pack_B_3VL, pack_A_fp16_8row, etc.

#ifndef PF_OFF_BYTES
#  define PF_OFF_BYTES 512
#endif

#define L2PIN_STR2(x) #x
#define L2PIN_STR(x)  L2PIN_STR2(x)

#if PF_OFF_BYTES > 0
#  define L2PIN_PRFM_B "prfm pldl2keep, [x22, #" L2PIN_STR(PF_OFF_BYTES) "]\n"
#else
#  define L2PIN_PRFM_B ""
#endif
...
void sve_interleaved_fp16_mla_8x3VL_l2pin_nt(
