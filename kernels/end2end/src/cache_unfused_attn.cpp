...
/*
 * cache_unfused_attn.cpp — Plan B timing (no KV prepack) — Optimized unfused attention baseline (VL=4, gem5)
 *
 * Derived from 12_attn_baseline_seq_vl4.cpp with two key optimizations:
 *
 *   OPT 1: FUSED QK SCATTER — After each micro-kernel call (8 rows × 1 N-block),
 *           scatter Cpanel → scores[T,T_pad] with inline scale IMMEDIATELY,
 *           rather than accumulating all M-blocks then scattering in a separate pass.
 *           The original v12 already does per-M-block scatter, but it times scatter
 *           separately. Here we make the fusion explicit and eliminate the timing
 *           overhead of separate Clock::now() calls per M-block scatter.
 *
 *   OPT 2: FUSED PV SCATTER — Same approach for the PV GEMM: after each micro-kernel
 *           call for one M-block, immediately scatter/accumulate Cpanel → out[T,d].
 *
 *   OPT 3: KC_pv=256 — Use larger K-tiles for PV GEMM. Each pack_A reads 256 cols
 *           from scores (P matrix). A_panel = MC*KC*2 = 128*256*2 = 64KB fits in L1.
 *           This reduces K-tile iterations from 8 (KC=64) to 2, cutting pack_A overhead
 *           by 4x and improving B-panel reuse.
 *
 *   KEPT:  Same GEMM micro-kernel (8x3VL), same SVE softmax (3-pass, 2-unrolled exp),
 *          same gather-transpose pack_K, same prepack_B_knm. These are already SOTA.
 *
 * Pipeline per head (same as v12, no flash attention, no SPM):
 ...
 *
 * Build:
 *   $CROSS_CXX -O3 -std=c++17 -march=armv8.2-a+sve+fp16 -static -DGEM5 -DGEM5_SE \
 *       -I$GEM5_INC seq_cache_opt.cpp $M5OP -lm -o bin/seq_cache_opt
 *
 * Usage: ./bin/seq_cache_opt [T] [H_q] [H_kv] [d] [n_iter] [MC_qk] [KC_pv] [MC_pv] [phase] <causal|noncausal>
