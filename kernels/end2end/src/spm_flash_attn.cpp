/*
 * SPM FlashAttention with a j-outer loop and fp16/fp32 SVE conversion.
 *
 * Each K/V block is copied to SPM once and reused across its i-block group.
        ...
        for (int hq = 0; hq < H_q; ++hq) {
            ...
            for (int g = 0; g < n_groups; ++g) {
                ...
                for (int j = 0; j < T; j += Bc) {
                    ...
                    // Stage K and V for this head and j-block. The QK kernel
                    // reuses these SPM rows, so each head restages its inputs.
                    { auto _t = PHASE_T0();
                    ...
                    // Stage V together with K. At VL16 the cache-compatible PV
                    // kernel consumes V, while this copy retains the full K+V
                    // placement traffic in the measured path.
...
#if defined(VL_16)
                            // VL=16 uses the cache-compatible PV kernel. Its 256-B
                            // vector access spans consecutive rows, whereas the
                            // staged V layout is interleaved across SPM ways.
