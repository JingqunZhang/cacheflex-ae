...
/*
 * 13c_flash_opt_vl4.cpp — Optimized cache Flash Attention (VL=4, d=64)
 *
 * Improvements over 13/13b:
 *   1. Dedicated 8×64 PV kernel (no 33% waste from padding to 96)
 *   2. Adaptive Bc (variable, not hardcoded NT=96)
 *   3. QK kernel with ldc stride (write directly to S_panel[br, bc])
 *
 * Middle comparison point between:
 *   12_attn_baseline:       unfused 3-step, scores[T×T] in DRAM
 *   13 (this):              fused flash attn, no T×T buffer, K/V from L2/DRAM
 *   11_flash_attn_kv_spm:   fused flash attn, K+V pinned in SPM
 *
 ...
 *
 * Build (1-step, no SPM compiler):
 *   $CROSS_CXX -O3 -std=c++17 -march=armv8.2-a+sve+fp16 -static \
 *     -DGEM5 -DGEM5_SE -I$GEM5_INC 13_flash_attn_nospm_vl4.cpp $M5OP -lm \
 *     -o bin/13_flash_attn_nospm_vl4
 *
    ...
    bool causal = false, FlashTiming* ft = nullptr)
{
    ...
    for (int i = 0; i < T; i += Br) {
        ...
        if (ft) ft->t_packA_q += us_since(_t); }
        ...
        for (int j = 0; j < T; j += Bc) {
            ...
            for (int r = 0; r < br; ++r) {
                ...
                svfloat32_t vmax = svdup_n_f32(-1e30f);
                for (size_t c = 0; c < (size_t)bc; c += VL32) {
                    svbool_t pg32 = svwhilelt_b32_u64(c, (size_t)bc);
                    svbool_t pg16 = svwhilelt_b16_u64(c, std::min(c+VL32, (size_t)bc));
                    svfloat32_t vs = svmul_n_f32_x(pg32,
                        svcvt_f32_f16_x(pg32, svreinterpret_f16_u32(svld1uh_u32(pg32, (const uint16_t*)(s_row+c)))), scale);
                    vmax = svmax_f32_m(pg32, vmax, vs);
                }
                float m_ij = svmaxv_f32(svptrue_b32(), vmax);
                float m_old = m_vec[r];
                ...
                for (size_t c = 0; c < (size_t)bc; c += VL32) {
                    svbool_t pg32 = svwhilelt_b32_u64(c, (size_t)bc);
                    svbool_t pg16 = svwhilelt_b16_u64(c, std::min(c+VL32, (size_t)bc));
                    svfloat32_t vs = svmla_n_f32_x(pg32, svdup_n_f32(-m_new),
                        svcvt_f32_f16_x(pg32, svreinterpret_f16_u32(svld1uh_u32(pg32, (const uint16_t*)(s_row+c)))), scale);
                ...
                m_vec[r] = m_new;
                l_vec[r] = alpha * l_vec[r] + svaddv_f32(svptrue_b32(), vsum);
                scale_f32_sve(O_acc + (size_t)r * d, alpha, (size_t)d);
            }
            if (ft) ft->t_sm += us_since(_t); }
            ...
            for (size_t mb = 0; mb < ablocks; ++mb) {
                ...
                if (ft) ft->t_pv_packA += us_since(_t); }
                ...
                if (ft) ft->t_pv_kern += us_since(_t); }
            }
        }
        ...
        for (int r = 0; r < br; ++r) {
            ...
            size_t VL32 = svcntw(), k = 0;
            for (; k + VL32 <= (size_t)d; k += VL32) {
                svbool_t pg32 = svptrue_b32();
                svbool_t pg16 = svwhilelt_b16_u64(k, k+VL32);
            ...
            if (k < (size_t)d) {
                ...
                svbool_t pg16 = svwhilelt_b16_u64(k, (size_t)d);
                svst1h_u32(pg32, (uint16_t*)(o_dst+k), svreinterpret_u32_f16(svcvt_f16_f32_x(pg32, svmul_n_f32_x(pg32, svld1_f32(pg32, o_src+k), inv_l))));
            }
        }
        if (ft) ft->t_fin += us_since(_t); }
    }
}
