...
rh_launch() {
    ...
    rh__slot_wait
    echo "[run ] $out"
    # Per-cell wall-clock cap so one hung simulation cannot stall a reviewer
    # indefinitely.  Override with RH_CELL_TIMEOUT (seconds); default 24h —
...
rh_validate_gem5_cells() {
    ...
    for out in "${RH_OUTS[@]}"; do
        ...
        if ! grep -q 'Exiting @ tick' "$log"; then
            # Known T=4096 exit behavior (docs/CLAIMS.md Known issues): a few
            # vl4 llama T=4096 cells complete the ROI timing receipt and the
            # stats dump but do not print the clean-exit line; the shipped
            # reference exhibits the same behavior. Accept exactly those
            # cells — the stats-dump and crash-marker checks above/below
            # still apply to them.
            case "$out" in
                */vl4/llama_T4096_cflash_br64_bc576|*/vl4/llama_T4096_unfused|*/vl4/llama_T4096_spm_unfused)
                    echo "NOTE: $out lacks a clean gem5 exit line (known T=4096 behavior; stats and timing receipt verified)" >&2
                    ;;
                *)
