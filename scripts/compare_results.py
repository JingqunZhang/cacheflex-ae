#!/usr/bin/env python3
"""Compare fresh plot data with the paper results at a fixed 5% limit."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare Figure 2/7/8/9 and Table 4 data against "
...
def signed_number(value: Any, where: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise SchemaError(f"{where}: expected a number")
    result = float(value)
    if not math.isfinite(result):
        raise SchemaError(f"{where}: expected a finite number")
    return result
...
def add_signed_metric(
    ...
    if name in metrics:
        raise SchemaError(f"{name}: duplicate metric")
    metrics[name] = (signed_number(value, name), anchor)
...
def data_metrics(
    ...
    try:
        ...
        elif filename == "table4_data.json":
            for record in document["records"]:
                prefix = (
                    f"VL{record['vector_length']}"
                    f"/T{record['sequence_length']}"
                )
                add_metric(
                    metrics,
                    f"{prefix}/cache_time_us",
                    record["cache_time_us"],
                )
                add_metric(
                    metrics,
                    f"{prefix}/spm_time_us",
                    record["spm_time_us"],
                )
                add_signed_metric(
                    metrics,
                    f"{prefix}/measured_gain_pct",
                    record["measured_gain_pct"],
                    anchor=0.0,
                )
                add_signed_metric(
                    metrics,
                    f"{prefix}/reported_gain_pct",
                    record["reported_gain_pct"],
                    anchor=0.0,
                )
        else:
            raise SchemaError(f"unsupported plot-data file: {filename}")
    except (KeyError, TypeError, ZeroDivisionError) as error:
        raise SchemaError(f"{filename}: malformed plot data: {error}") from error
    return metrics
...
def compare_metrics(
    ...
    for name in sorted(expected_keys & actual_keys):
        ...
        if expected_value == 0.0:
            ...
            continue
        relative_error = 100.0 * difference / abs(expected_value)
...
def main() -> int:
    ...
    try:
        for filename in DATA_FILES:
            ...
            expected_metrics = data_metrics(expected, filename)
            actual_metrics = data_metrics(actual, filename)
