...
# Keep the publication asset's physical canvas so fig2_real.pdf can replace
...
if not BEST_PATH.is_file():
    raise SystemExit(f'ERROR: missing frozen selection: {BEST_PATH}')
...
def best_cache(dirname, vl, cap, ld):
    """Read the unique cache cell frozen for this configuration."""
    pattern = re.compile(
...
def best_spm(dirname, vl):
    """Read the unique CacheFlex cell frozen for this vector length."""
