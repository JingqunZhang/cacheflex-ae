...
OUTDIR="$(pwd)/results_kc_sweep_vl4"
mkdir -p "$OUTDIR"
...
echo "============================================================"
echo "  VL=4 KC sweep, MAX_JOBS=$MAX_JOBS"
echo "============================================================"

for kc in 128 256 512 1024 2048; do run kc${kc}_W1_v1 "$BIN_V1" 4 128 4096 11008 $kc 128; done
for kc in 128 256 512 1024;      do run kc${kc}_W2_v1 "$BIN_V1" 4 256 4096 4096 $kc 256; done
for kc in 128 256 384 512 1024;  do run kc${kc}_W3_v1 "$BIN_V1" 4 256 2048 2048 $kc 256; done
for kc in 128 256 384 512 768;   do run kc${kc}_W4_v1 "$BIN_V1" 4 512 768 768 $kc 784; done
for kc in 256 512 1024 2048;     do run kc${kc}_W5_v1 "$BIN_V1" 4 2048 2048 2048 $kc 2048; done
for kc in 32 64;                 do run kc${kc}_W6_v1 "$BIN_V1" 4 2048 64 2048 $kc 1024; done
for kc in 64 128 256;            do run kc${kc}_W7_v1 "$BIN_V1" 4 784 256 1024 $kc 784; done
for kc in 128 256 512 1024;      do run kc${kc}_W1_v3 "$BIN_V3_DIR/v3_fused_vl4" 4 128 4096 11008 $kc 128; done
for kc in 128 256 512 1024;      do run kc${kc}_W2_v3 "$BIN_V3_DIR/v3_fused_vl4" 4 256 4096 4096 $kc 256; done
for kc in 128 256 512 1024;      do run kc${kc}_W3_v3 "$BIN_V3_DIR/v3_fused_vl4" 4 256 2048 2048 $kc 256; done
for kc in 128 256 512 768;       do run kc${kc}_W4_v3 "$BIN_V3_DIR/v3_fused_vl4" 4 512 768 768 $kc 784; done
for kc in 256 512 1024;          do run kc${kc}_W5_v3 "$BIN_V3_DIR/v3_fused_vl4" 4 2048 2048 2048 $kc 1024; done
for kc in 32 64;                 do run kc${kc}_W6_v3 "$BIN_V3_DIR/v3_fused_vl4" 4 2048 64 2048 $kc 1024; done
for kc in 64 128 256;            do run kc${kc}_W7_v3 "$BIN_V3_DIR/v3_fused_vl4" 4 784 256 1024 $kc 784; done
if ! wait_for_jobs; then
    exit 1
fi
echo "VL4 KC SWEEP DONE"
