...
echo "============================================================"
echo "  VL=16 KC sweep, MAX_JOBS=$MAX_JOBS"
echo "============================================================"

for kc in 32 64 128 256 512 1024; do run kc${kc}_W1_v1 "$BIN_V1" 16 128 4096 11008 $kc 128; done
for kc in 32 64 128 256 512 1024; do run kc${kc}_W2_v1 "$BIN_V1" 16 256 4096 4096 $kc 256; done
for kc in 32 64 128 256 512 1024; do run kc${kc}_W3_v1 "$BIN_V1" 16 256 2048 2048 $kc 256; done
for kc in 32 64 128 256 384;      do run kc${kc}_W4_v1 "$BIN_V1" 16 512 768 768 $kc 256; done
for kc in 32 64 128 256 512 1024; do run kc${kc}_W5_v1 "$BIN_V1" 16 2048 2048 2048 $kc 512; done
for kc in 32 64;                  do run kc${kc}_W6_v1 "$BIN_V1" 16 2048 64 2048 $kc 512; done
for kc in 32 64 128 256;          do run kc${kc}_W7_v1 "$BIN_V1" 16 784 256 1024 $kc 784; done
# CacheFlex KC values respect the VL16 SPM capacity.
for kc in 64 128 256 320; do run kc${kc}_W1_v3 "$BIN_V3_DIR/v3_fused_vl16" 16 128 4096 11008 $kc 128; done
for kc in 64 128 256 320; do run kc${kc}_W2_v3 "$BIN_V3_DIR/v3_fused_vl16" 16 256 4096 4096 $kc 256; done
for kc in 64 128 256 320; do run kc${kc}_W3_v3 "$BIN_V3_DIR/v3_fused_vl16" 16 256 2048 2048 $kc 256; done
for kc in 64 128 256 320; do run kc${kc}_W4_v3 "$BIN_V3_DIR/v3_fused_vl16" 16 512 768 768 $kc 256; done
for kc in 64 128 256 320; do run kc${kc}_W5_v3 "$BIN_V3_DIR/v3_fused_vl16" 16 2048 2048 2048 $kc 512; done
for kc in 32 64;          do run kc${kc}_W6_v3 "$BIN_V3_DIR/v3_fused_vl16" 16 2048 64 2048 $kc 512; done
for kc in 64 128 256;     do run kc${kc}_W7_v3 "$BIN_V3_DIR/v3_fused_vl16" 16 784 256 1024 $kc 784; done
